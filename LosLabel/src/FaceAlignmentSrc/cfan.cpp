...
#include "FaceAlignmentInc/cfan.h"
#include <string.h>
#include <algorithm>
/** A constructor.
...
void CCFAN::InitModel(const char *model_path)
{
    ...
    if (!fread(mean_shape_, sizeof(float), pts_num_ * 2, fp)) return;

    /*Load the parameters of the first local stacked autoencoder network*/
    if (!fread(&lan1_size_, sizeof(int), 1, fp)) return;
    lan1_structure_ = new int[lan1_size_];
    if (!fread(lan1_structure_, sizeof(int), lan1_size_, fp)) return;
    ...
    for (int i = 0; i < lan1_size_ - 1; i++)
    {
        ...
        if (!fread(lan1_w_[i], sizeof(float), layer_size, fp)) return;

        lan1_b_[i] = new float[lan1_structure_[i + 1]];
        if (!fread(lan1_b_[i], sizeof(float), lan1_structure_[i + 1], fp)) return;
    }

    /*Load the parameters of the second local stacked autoencoder network*/
    if (!fread(&lan2_size_, sizeof(int), 1, fp)) return;
    lan2_structure_ = new int[lan2_size_];
    if(!fread(lan2_structure_, sizeof(int), lan2_size_, fp)) return;
    ...
    for (int i = 0; i < lan2_size_ - 1; i++)
    {
        ...
        if (!fread(lan2_w_[i], sizeof(float), layer_size, fp)) return;

        lan2_b_[i] = new float[lan2_structure_[i + 1]];
        if (!fread(lan2_b_[i], sizeof(float), lan2_structure_[i + 1], fp)) return;
    }
    fclose(fp);
}
